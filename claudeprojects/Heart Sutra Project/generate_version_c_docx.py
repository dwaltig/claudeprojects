...
def _process_inline_formatting(paragraph, text):
    ...
    # We use a pattern that catches the delimiters to preserve them if needed, 
    # but here we use it to alternate between formatted and unformatted text.
    pattern = r'(\*\*\*.*?\*\*\*|\*\*.*?\*\*|\*.*?\*)'
...
if __name__ == "__main__":
    input_file = "ARTICLE_Hellhounds_VERSION_C.md"
    if not os.path.exists(input_file):
        print(f"Error: {input_file} not found.")
    else:
        markdown_to_docx(input_file, "ARTICLE_Hellhounds_VERSION_C_AUTHOR.docx", blind=False)
        markdown_to_docx(input_file, "ARTICLE_Hellhounds_VERSION_C_BLIND.docx", blind=True)
        if os.path.exists("JAR_RESPONSE_LETTER_VERSION_C.md"):
             markdown_to_docx("JAR_RESPONSE_LETTER_VERSION_C.md", "JAR_RESPONSE_LETTER_VERSION_C.docx", blind=False)
