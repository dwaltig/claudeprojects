...
def extract_chapter_content(filepath):
    ...
    prose_text = re.sub(r'\[\^[\d]+\]', '', prose_text)
...
def main():
    ...
    for ch_num in range(1, 29):
        ...
        print(f"Processing Ch {ch_num}...", end=" ")

        try:
            title, prose, verse = extract_chapter_content(filepath)

            # Create simple HTML content - properly escaped for JSON
            html_content = f'<div class="chapter-title">Chapter {ch_num}: {titles[ch_num]}</div><div>'

            if prose:
                paras = [p.strip() for p in prose.split('\n\n') if p.strip()]
                for para in paras[:2]:  # First 2 paragraphs
                    # Escape quotes for JSON
                    para_escaped = para.replace('"', '\\"')
                    html_content += f'<p>{para_escaped}</p>'

            if verse:
                html_content += '<div class="verse">'
                verse_lines = [l.strip() for l in verse.split('\n') if l.strip()]
                html_content += '<br>'.join(verse_lines[:6])
                html_content += '</div>'
    ...
    # Save to JSON
    output_path = base_path / 'chapters_data.json'
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(chapters_data, f, ensure_ascii=False, indent=2)

    print(f"\n✓ Saved {len(chapters_data)} chapters to chapters_data.json")
