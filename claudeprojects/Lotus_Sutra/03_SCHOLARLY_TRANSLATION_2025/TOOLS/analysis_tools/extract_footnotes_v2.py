...
def extract_all_footnotes(content):
    ...
    # Pattern: [^1]: text that might span lines
    lines = content.split('\n')
    i = 0
    while i < len(lines):
        line = lines[i]
        ...
        if match:
            ...
            # Collect continuation lines (indented lines following the footnote)
            i += 1
            while i < len(lines):
                next_line = lines[i]
                # If it's indented or continues text, add it
                if next_line.startswith('    ') or (next_line and not next_line.startswith('[^') and not next_line.startswith('#')):
...
def extract_chapter_with_footnotes(filepath):
    ...
    all_footnotes = extract_all_footnotes(content)
    ...
    prose_html = markdown_to_html_with_footnotes(prose_text, all_footnotes)
    verses = extract_verses(verse_text)

    return prose_html, verses, all_footnotes

def markdown_to_html_with_footnotes(text, footnotes):
    """Convert markdown to HTML with interactive footnotes."""
    ...
    # Convert footnote references [^1] to interactive tooltips
    def replace_footnote(match):
        num = match.group(1)
        if num in footnotes:
            # Escape quotes and newlines for data attribute
            footnote_text = footnotes[num].replace('"', '&quot;').replace('\n', ' ')
            return f'<span class="footnote-ref" data-note="{num}" data-text="{footnote_text}" onclick="showFootnoteTooltip(event)" onmouseover="showFootnoteTooltip(event)"><sup>{num}</sup></span>'
        else:
            # No definition, just show number
